...
HL_TARGET=host HL_SEED=root PIPELINE_SEED=0 HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 make build
...
for ((b=0;b<10;b++)); do
    ...
    for ((p=0;p<$PIPELINES;p++)); do
        ...
        echo "HL_TARGET=host HL_SEED=root PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 make build 2>&1 | grep -v Nothing.to.be.done"
        ...
        for ((s=0;s<$SCHEDULES;s++)); do
            ...
            echo "HL_TARGET=host HL_SEED=$s PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 make build 2>&1 | grep -v Nothing.to.be.done"
        done
    ...
    for ((p=0;p<$PIPELINES;p++)); do
        ...
        F=bin/host/pipeline_${P}_${STAGES}/schedule_root_50_1/times.txt
        if [ ! -f $F ]; then HL_TARGET=host HL_SEED=root PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 HL_NUM_THREADS=24 numactl --cpunodebind=1 make bench 2>&1 | grep -v "Nothing to be done"; fi
        ...
        for ((s=0;s<$SCHEDULES;s++)); do
            ...
            F=bin/host/pipeline_${P}_${STAGES}/schedule_${s}_50_1/times.txt
            if [ ! -f $F ]; then HL_TARGET=host HL_SEED=$s PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 HL_NUM_THREADS=24 numactl --cpunodebind=1 make bench 2>&1 | grep -v "Nothing to be done"; fi
    ...
    cat results/files_${b}.txt | while read F; do echo $(grep '^Optimal state with cost' ${F/times/stderr} | cut -d' ' -f4 | cut -d: -f1); done  > results/costs_${b}.txt
done
