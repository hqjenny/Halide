...
benchmark_sample() {
    ...
    for ((SAMPLE_IDX=0;SAMPLE_IDX<$LOOP_BOUND;SAMPLE_IDX++)); do
        FEAT_FILE=${HL_JENNY_DIR}/${SAMPLE_IDX}.featurization
        RESULT_TXT=${HL_JENNY_DIR}/${SAMPLE_IDX}.txt
        SAMPLE_FILE=${HL_JENNY_DIR}/${SAMPLE_IDX}.jen.sample
        ...
        #${AUTOSCHED_BIN}/featurization_to_sample ${FEAT_FILE} $R $P $S ${SAMPLE_FILE} || echo "featurization_to_sample failed for ${D} (probably because benchmarking failed)"
        ${AUTOSCHED_BIN}/featurization_to_sample_jenny ${FEAT_FILE} $R $P $S ${SAMPLE_FILE} || echo "featurization_to_sample failed for ${D} (probably because benchmarking failed)"
    done 
}
...
for ((BATCH_ID=$((FIRST+1));BATCH_ID<$((FIRST+1+NUM_BATCHES));BATCH_ID++)); do
    ...
    for ((EXTRA_ARGS_IDX=0;EXTRA_ARGS_IDX<${#GENERATOR_ARGS_SETS_ARRAY[@]};EXTRA_ARGS_IDX++)); do
        ...
        if [ "$RETRAIN" == "true" ]; then
            ...
            find ${SAMPLES} -name "*.jen.sample" | \
                PIPELINE=${PIPELINE} \
                ${AUTOSCHED_BIN}/retrain_cost_model_jenny \
                    --epochs=${BATCH_SIZE} \
                    --rates="0.0001" \
                    --num_cores=32 \
